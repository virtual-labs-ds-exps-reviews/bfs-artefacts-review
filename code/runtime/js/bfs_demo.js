...
document.getElementById("ins").innerHTML = '<b>'+bfs_demo.src +'</b>'+ " is selected as source node <br>Click on <b>Start</b> to start the BFS Demo with the source node"+'<b>'+bfs_demo.src+'</b>'
...
function clicking() {
    if (bfs_demo.resetter == 1)return;
        ...
        if (bfs_demo.res[bfs_demo.count].id == undefined && bfs_demo.par[bfs_demo.it] == bfs_demo.count) {
            clickednode2(bfs_demo.res[bfs_demo.count]);
            bfs_demo.seq[bfs_demo.cnt++] = clickednodeid(bfs_demo.res[bfs_demo.count]);
            document.getElementById("ins").innerHTML = "Node <b>"+(bfs_demo.res[bfs_demo.count]-1)+"</b> is the current node";
            bfs_demo.it++;
            bfs_demo.count++;
        } else if (bfs_demo.res[bfs_demo.count].id == undefined && bfs_demo.par[bfs_demo.it] != bfs_demo.count) {
            document.getElementById("ins").innerHTML = "Node <b>"+(bfs_demo.res[bfs_demo.count]-1)+"</b> is explored completely";
            clickednode3(bfs_demo.res[bfs_demo.count]);
            bfs_demo.count++;
        } else {
            document.getElementById("ins").innerHTML = "Node <b>"+clickednodeid(bfs_demo.res[bfs_demo.count].node)+"</b> is visited";
            clickededge1(bfs_demo.res[bfs_demo.count].id);
            clickednode(bfs_demo.res[bfs_demo.count].node);
            bfs_demo.count++;
        }
    }
    else if(bfs_demo.count==bfs_demo.res.length && bfs_demo.start==1 && bfs_demo.first==1){
        document.getElementById("ins").innerHTML = "BFS is done on the node <b>"+bfs_demo.src+"</b> !!!<br>Sequence of nodes visited on performing BFS on node "+bfs_demo.src+" : "+bfs_demo.seq;
    }
}
function reset() {
...
function start(){
    if(bfs_demo.start == 0){
        if(bfs_demo.src1==0){
            document.getElementById("ins").innerHTML = "Select the source node from where you want to start the BFS Demo";
        }
        else{
        document.getElementById("ins").innerHTML = "BFS Demo started on node "+"<b>"+bfs_demo.src+"</b>";    
        bfs_demo.first = 1;
        bfs_demo.start = 1;
        bfs_demo.res = g.bfs(bfs_demo.src1);
        document.getElementById("start").value = "Next";
        bfs_demo.time = setInterval(clicking, 3000 - document.getElementById("interval").value);}
    }
...
if (bfs_demo.time != 0) { clearInterval(bfs_demo.time); }
